...
def agent(tmp_path, monkeypatch):
    ...
    En CI no hay un servidor Ollama escuchando en localhost:11434.
    Para tests unitarios, mockeamos la llamada a Ollama y devolvemos
    una respuesta determinista tipo 'echo'.
    """
    a = OllamaAgent(
        model="qwen2.5:7b",
    ...
    def _fake_call_ollama(messages, tools=None):
        # Toma el último mensaje del usuario y devuelve un echo.
        last_user = ""
        ...
        # agent.run() antepone contexto con "\n\n".
...
def test_cache(agent):
    # Aunque no exista un caché real, el mock es determinista y el test valida estabilidad.
